...
for csv_path in tqdm(csv_paths):
    print(csv_path)
    output_dir = os.path.join(root, "..", "processed", "nbayes_models")
...
for subdir in ["chembl", "coadd", "spark"]:
    if not os.path.exists(os.path.join(root, "..", "processed", "nbayes_models", subdir)):
        continue
    for model_dir in os.listdir(os.path.join(root, "..", "processed", "nbayes_models", subdir)):
        if not os.path.exists(os.path.join(root, "..", "processed", "nbayes_models", subdir, model_dir, "results.json")):
            shutil.rmtree(os.path.join(root, "..", "processed", "nbayes_models", subdir, model_dir))
            continue
        with open(os.path.join(root, "..", "processed", "nbayes_models", subdir, model_dir, "results.json"), "r") as f:
            results = json.load(f)
...
pd.DataFrame(R).to_csv(os.path.join(root, "..", "processed", "01_nbayes_models_summary.csv"), index=False)
